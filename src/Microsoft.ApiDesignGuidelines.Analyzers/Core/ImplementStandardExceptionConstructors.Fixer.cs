...
namespace Microsoft.ApiDesignGuidelines.Analyzers
{
    ...
    public sealed class ImplementStandardExceptionConstructorsFixer : CodeFixProvider
    {
        ...
        private async Task<Document> AddConstructorsAsync(Document document, IEnumerable<Diagnostic> diagnostics, SyntaxNode root, CancellationToken cancellationToken)
        {
            ...
            TextSpan diagnosticSpan = diagnostics.First().Location.SourceSpan; // All the diagnostics are reported at the same location -- the name of the declared class -- so it doesn't matter which one we pick
